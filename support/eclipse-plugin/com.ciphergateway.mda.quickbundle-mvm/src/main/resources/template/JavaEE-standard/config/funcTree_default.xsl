...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:str="http://www.quickbundle.org" xmlns:fn="http://www.w3.org/2005/04/xpath-functions" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xdt="http://www.w3.org/2005/02/xpath-datatypes" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	rmCodeGeneration<xsl:value-of select="$tableFormatNameUpperFirst"/>_maintenance = insDoc(node_generated_module, gLnk("0","<xsl:value-of select="$charNbsp"/><xsl:value-of select="$tableNameDisplay"/>", "<xsl:value-of select="$charLt"/>%=request.getContextPath()%>/<xsl:value-of select="$tableDirName"/>", "ftv2link.gif"));
	</xsl:template>
</xsl:stylesheet>

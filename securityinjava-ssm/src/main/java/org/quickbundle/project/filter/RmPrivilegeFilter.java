...
public class RmPrivilegeFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            { // P3P http://www.w3.org/TR/p3pdeployment P3P:
              // policyref="http://catalog.example.com/P3P/PolicyReferences.xml",CP="NON DSP
              // COR CURa ADMa DEVa CUSa TAIa OUR SAMa IND"
              // •policyref - this field gives the location of the policy reference file; its
              // value is a URL where the site´s policy reference file can be located
              // •CP - this field gives a compact policy for the resource (URL) that has been
              // requested. Its value is the text of the compact P3P policy.
              // res.setHeader("P3P","CP=\"NON DSP COR CURa ADMa DEVa CUSa TAIa OUR SAMa
              // IND\"");
            }
            ...
            if (RmConfig.getSingleton().isUserDemoMode()) { // demo模式不校验登录
                ...
                // RmJspHelper.setProfile(req, res, "toUrl",
                // fullUrl.substring(req.getContextPath().length()), 60);
        ...
        if (!isInitIgnoreUrlConf) { // 双检锁初始化
            synchronized (lockIgnoreUrlConf) {
                if (!isInitIgnoreUrlConf) {
                    List<Node> lUrl = RmLoadConfig.getRmDoc()
                            .selectNodes("/rm/org.quickbundle.project.filter.RmPrivilegeFilter/ignoreUrls/url");
                    for (Node node : lUrl) {
                        sIgnoreUrl.add(node.getText().trim());
                    }
                    validBsUrlMatch = Pattern.compile(RmLoadConfig.getRmDoc()
                            .valueOf("/rm/org.quickbundle.project.filter.RmPrivilegeFilter/validBsUrlMatch/text()"));
